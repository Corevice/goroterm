...
class CustomTextEditState extends State<CustomTextEdit> with TextInputClient {
  ...
  /// true の間は updateEditingValue を無視する（リセット遅延中の再トリガー防止）
  bool _pendingReset = false;

  /// 送信済みテキスト位置。リセットがスキップされた場合の delta 計算ベース。
  String _sentBase = '';
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    // delta 計算: _sentBase が設定されていればそこからの差分を使う
    final base = _sentBase.isNotEmpty ? _sentBase : _initEditingState.text;

    if (_currentEditingState.text.length < base.length) {
      widget.onDelete();
    } else if (_currentEditingState.text.length > base.length) {
      final textDelta = _currentEditingState.text.substring(base.length);
    ...
    // 送信済みテキストを記録
    _sentBase = _currentEditingState.text;

    // Reset editing state if composing is done.
    // Delay by one frame to allow the platform to process pending keystrokes.
    if (_currentEditingState.text != _initEditingState.text) {
      _pendingReset = true;
      // 50ms 遅延: 確定直後に始まる新しい composing を待つ。
      // addPostFrameCallback だと同フレーム内でも間に合わない場合がある。
